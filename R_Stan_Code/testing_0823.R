...
#CFR = rep(0, A)
#FR1 = c(2.6e-05, 1.48e-04*204.46/(204.46+209.58))
...
# assume DeathsByAge is a data frame with 658 rows (days) × 7 columns (age groups)

# number of rows per block
block_size <- 30
n_days <- nrow(DeathsByAge)

# number of full 30-day blocks + last shorter block (28 days)
n_blocks <- ceiling(n_days / block_size)

# split row indices into blocks (last block gets remaining 28 days)
block_indices <- split(seq_len(n_days),
                       ceiling(seq_along(seq_len(n_days)) / block_size))
# manually adjust last block to contain only 28 days
block_indices[[length(block_indices)]] <-
  tail(block_indices[[length(block_indices)]], 28)

# compute mean for each block and age group
avg_list <- lapply(block_indices, function(idx) {
  colMeans(DeathsByAge[idx, , drop = FALSE], na.rm = TRUE)
})

avg_list1 <- lapply(block_indices, function(idx) {
  colMeans(CasesByAge[idx, , drop = FALSE], na.rm = TRUE)
})


# convert list to data frame (22 rows × 7 columns)
DeathsByAge_avg <- do.call(rbind, avg_list)
CasesByAge_avg <- do.call(rbind, avg_list1)

# check dimension: should be 22 × 7
dim(DeathsByAge_avg)
dim(CasesByAge_avg)
CFR = DeathsByAge_avg/CasesByAge_avg

CFRMIN = min(CFR[CFR != 0])
CFR[CFR == 0] = CFRMIN
View(CFR)
...
start.time <- Sys.time()
file <- "C:/Users/user/Desktop/Covid19-Age/Covid19-Age_specific/Singapore/testing_0823.stan"
#cmdstan_path()
...
print(time.taken)
m_hier$save_object(file = "cases_hospitalizations_ihr0.8_phi21_3_TimeVaryInfe_60_fit_1202.RDS")
...
ggplot(time_series_data1_hosp, aes(x = time, y = values)) +
  ...
  theme(
    axis.text = element_text(size = 42, face = "bold"),
