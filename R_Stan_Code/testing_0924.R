...
# Serial Interval
serial_interval <- readRDS("C:/Users/user/Desktop/Covid19-Age/Covid19Data/serial-interval.rds")
...
serial_interval_Delta <- readRDS("C:/Users/user/Desktop/Covid19-Age/Covid19Data/serial_interval_Delta.rds")
#serial_interval_Delta <- readRDS("/home/jingyan/Covid19SG/Dataset/serial_interval_Delta.rds")
SI_Delta = serial_interval_Delta

serial_interval_Omicron <- readRDS("C:/Users/user/Desktop/Covid19-Age/Covid19Data/serial_interval_Omicron.rds")
...
# Actual deaths
CovidDeathSG <- read_excel("C:/Users/user/Desktop/Covid19-Age/Covid19Data/CovidDeathSG212223.xlsx")
...
###############
CovidLocalSG <- read_excel("C:/Users/user/Desktop/Covid19-Age/Covid19Data/CovidLocalSG212223.xlsx")
CovidImportSG <- read_excel("C:/Users/user/Desktop/Covid19-Age/Covid19Data/CovidImportSG212223.xlsx")
...
#####################################################
CovidActiveHospSG <- read_excel("C:/Users/user/Desktop/Covid19-Age/Covid19Data/CovidActiveHospSG212223.xlsx")
CovidNewHospSG <- read_excel("C:/Users/user/Desktop/Covid19-Age/Covid19Data/CovidNewHospSG212223.xlsx")
...
ConMat = read_excel("C:/Users/user/Desktop/Covid19-Age/Covid19Data/ConMat.xlsx")
...
ConMatLock = read_excel("C:/Users/user/Desktop/Covid19-Age/Covid19Data/ConMat_Lockdown.xlsx")
...
SumCase = apply(CasesByAge[(TVaccine:Seed),], 2, mean)
SdCase = apply(CasesByAge[(TVaccine:Seed),], 2, sd)
...
data <- data.frame(
  Category = c("0-14", "15-29", "30-39", "40-49", "50-59", "60-69", "70+"),
  Contribution_to_SARS_CoV2_cases_after_intervention_of_vaccine = SumCase / sum(SumCase),
  Share_in_population = popByAge,
...
# Reshape data for side-by-side bars using pivot_longer
data_long <- pivot_longer(data, 
                          cols = c(Contribution_to_SARS_CoV2_cases_after_intervention_of_vaccine, Share_in_population), 
                          names_to = "Bar_Type", 
                          values_to = "Value")
...
p <- ggplot(data_long, aes(x = Category, y = Value, fill = Bar_Type)) +
  ...
  geom_bar(data = subset(data_long, Bar_Type == "Share_in_population"),
           ...
           color = "black", fill = "white", size = 1, 
           position = position_dodge(width = 0.8),
           show.legend = TRUE) +  # Black border for white bars
  # Add blue bars for Contribution_to_SARS_CoV2_cases_after_intervention_of_vaccine
  geom_bar(data = subset(data_long, Bar_Type == "Contribution_to_SARS_CoV2_cases_after_intervention_of_vaccine"),
           stat = "identity", 
           color = "transparent", fill = "#56B4D3", size = 1,
           position = position_dodge(width = 0.8),
           show.legend = FALSE) +  # Light ocean blue color
  geom_bar(data = subset(data_long, Bar_Type == "Share_in_population"),
           stat = "identity", 
           color = "black", fill = "transparent", size = 1,
           position = position_dodge(width = 0.8),
  ...
  labs(title = "Estimated sources of SARS-CoV-2 cases, Singapore average from 1 September 2021 until 30 April 2023", 
       x = "Age group", y = "Percentage of population (%)") + 
  scale_fill_manual(values = c("Contribution_to_SARS_CoV2_cases_after_intervention_of_vaccine" = "#56B4D3", "Share_in_population" = "white")) + 
  # Theme adjustments
  theme(
    axis.text = element_text(size = 21, face = "bold"),   # Enlarge axis tick labels
    axis.title = element_text(size = 21, face = "bold"),  # Enlarge axis labels
...
for (a in 1:A) {
  UPPERCI_Infe[a] = SumInfe[a] + 1.96 * SdInfe[a] / sqrt(dim(InfeByAge[(Seed+TVaccine):(Seed+N2),])[1])
  LOWERCI_Infe[a] = SumInfe[a] - 1.96 * SdInfe[a] / sqrt(dim(InfeByAge[(Seed+TVaccine):(Seed+N2),])[1])
}
...
for (a in 1:A) {
  UPPERCI_Case[a] = SumCase[a] + 1.96 * SdCase[a] / sqrt(dim(CasesByAge[1:(TVaccine-1),])[1])
  LOWERCI_Case[a] = SumCase[a] - 1.96 * SdCase[a] / sqrt(dim(CasesByAge[1:(TVaccine-1),])[1])
}
...
p <- ggplot(data_long, aes(x = Category, y = Value, fill = Bar_Type)) +
  ...
  scale_fill_manual(
    values = c(
      "Infection_Contribution" = "#56B4D3",
      "Case_Contribution" = "#009E73",
      "Share_in_population" = "white"
    ),
    labels = c(
      "Infection_Contribution" = "Infections",
      "Case_Contribution" = "Cases",
      "Share_in_population" = "Population share"
    )
  ) +
  labs(
    title = "Estimated Sources of SARS-CoV-2 Infections and Cases (Post-Vaccine)",
    x = "Age Group",
