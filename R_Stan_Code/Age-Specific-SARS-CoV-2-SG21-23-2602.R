...
control_params <- list(adapt_delta = 0.9, max_treedepth = 11)
start.time <- Sys.time()
file <- "~/Desktop/Covid19-Age/R_and_Stan/Age-Specific-SARS-CoV-2-SG21-23-2602.stan"
...
dir.create("cmdstan_output", showWarnings = FALSE)
m_hier <- mod$sample(
  seed = 100,
  data=data_list,
  iter_warmup=500,
...
saveRDS(
  m_hier,
  file = "cases_hospitalizations_ihr_phi2_TimeVaryInfe_fit_0207.RDS",
  compress = FALSE
)
#m_hier$save_object(file = "cases_hospitalizations_ihr_phi2_TimeVaryInfe_fit_0207.RDS")
m_hier = cases_hospitalizations_ihr_phi2_TimeVaryInfe_fit_0207
m_hier$cmdstan_diagnose()
#m_hier$ess_per_transition <- m_hier$ess_bulk/m_hier$num_draws_sampling
#R0 <- as_draws_df(m_hier$draws())$phi
#para <- m_hier$draws(variables = "kappa")
#hist(
#  para,
#  breaks = 50,
#  probability = TRUE,
...
# Combine cases and hospitalizations PPC checks for one age group
# Convert to matrices
...
rep_hosps_age <- Neg_Binomial_hosp_mat[, cols_age]  # S x N

# --- Observed data ---
obs_hosps_age <- hospsByAge[1:N, Age]

# --- Predictive mean checks ---
obs_mean_hosps <- mean(obs_hosps_age)
rep_means_hosps <- rowMeans(rep_hosps_age)
pred_mean_hosps <- mean(rep_means_hosps)
bayes_p_mean_hosps <- mean(rep_means_hosps >= obs_mean_hosps)

# --- Predictive variance checks ---
obs_var_hosps <- var(obs_hosps_age)
rep_vars_hosps <- apply(rep_hosps_age, 1, var)
pred_var_hosps <- mean(rep_vars_hosps)
bayes_p_var_hosps <- mean(rep_vars_hosps >= obs_var_hosps)

# --- Print results ---
cat("Hospitalizations - mean: observed =", obs_mean_hosps, 
    ", predictive =", pred_mean_hosps, 
    ", Bayesian p-value =", bayes_p_mean_hosps, "\n")
cat("Hospitalizations - variance: observed =", obs_var_hosps, 
...
ggplot(time_series_data1_case, aes(x = time, y = values)) +
  ...
  geom_col(aes(x = time, y = observe), fill = "#FFC20A", alpha=0.6) +
  ...
  scale_x_date(
    limits = c(as.Date("2021-07-07"), as.Date("2023-04-27")),
    breaks = seq(as.Date("2021-07-07"), as.Date("2023-04-27"), by = "90 days")
  ) + 
