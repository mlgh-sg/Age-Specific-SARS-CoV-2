...
parameters {
  ...
  matrix<lower=0>[M,A] monthly_effect_ifr;
  matrix<lower=0>[M,A] monthly_effect_ihr;
...
transformed parameters {
  ...
  matrix<lower=0>[M,A] ICR;
  ICR = monthly_effect_ifr ./ CFR;
  array[N2+Seed] matrix<lower=0>[A, A] TimeVaryInfeByAge;
  // initial values of E_infeByAge
  E_infeByAge[1:N0,:] = e_cases_N0;
  for (t in 1:N0){
    TimeVaryInfeByAge[t] = rep_matrix(e_cases_N0[t,:]/A, A);
  }
  ...
  for (t in (N0+1):(TVaccine+Seed)){
    ...
    TimeVaryInfeByAge[t] = (rep_matrix(tmp_row_vector_A, A))' .* ConMatTran[WeekId[t]]*rho0;
    
    tmp_vector_A = tmp_vector_A + (E_infeByAge[(t-1),:])';
    suscep[:,t] = fmax_vector(1 - tmp_vector_A ./ Pop, 0);
    E_infeByAge[t,:] .*= (suscep[:,t])';
    E_infeByAge[t,:] .*= exp(log_relsusceptibility_age);
    TimeVaryInfeByAge[t] .*= rep_matrix((suscep[:,t])', A);
    TimeVaryInfeByAge[t] .*= rep_matrix(exp(log_relsusceptibility_age), A);
  ...
  E_caseByAge[1:30,:] = rep_matrix(ICR[1, :], 30) .*  E_infeByAge[(1+Seed):(30+Seed),:];
  row_vector[A] icr_t;
  row_vector[A] icr_noise_t = to_row_vector(icr_noise);  // ensure same type

  // --- loop for remaining days --- 
  for (t in 31:N2) {
  icr_t = ICR[to_int(ceil(t %/% 30)), :];  // assign current ICR inside loop
  E_caseByAge[t, ] =
    (rev_icr_daysSinceInfection[(N2 - (t - 1) + 1):N2] *
     E_infeByAge[(1 + Seed):(t - 1 + Seed), ]) .* 
    (icr_t .* icr_noise_t);
  }
  ...
  E_hospsByAge[1:30,:] = rep_matrix(monthly_effect_ihr[1, :], 30) .* E_infeByAge[(1+Seed):(30+Seed),:];
  row_vector[A] ihr_t;
  row_vector[A] ihr_noise_t = to_row_vector(ihr_noise);  // convert to row_vector

  for (t in 31:N2) {
    // Get current period's monthly_effect_ihr
    ihr_t = monthly_effect_ihr[to_int(ceil(t %/% 30)), :];  // cast to int for indexing
    // Compute expected hospitalizations by age
    E_hospsByAge[t, ] =
        (rev_ihr_daysSinceInfection[(N2 - (t - 1) + 1):N2] *
         E_infeByAge[(1 + Seed):(t - 1 + Seed), ]) .* 
        (ihr_t .* ihr_noise_t);
  }
...
model {
  ...
  monthly_effect_ifr[1,] ~ normal(1, 0.01);
  monthly_effect_ihr[1,] ~ normal(1, 0.01);
  ...
  for (a in 1:A) {
    ...
    monthly_effect_ifr[2:M,a] ~ normal(monthly_effect_ifr[1:(M-1),a], weekly_sd*sqrt(1-pow(weekly_rho,2)-pow(weekly_rho1,2)-2*pow(weekly_rho,2)*weekly_rho1/(1-weekly_rho1)));
    monthly_effect_ihr[2:M,a] ~ normal(monthly_effect_ihr[1:(M-1),a], weekly_sd*sqrt(1-pow(weekly_rho,2)-pow(weekly_rho1,2)-2*pow(weekly_rho,2)*weekly_rho1/(1-weekly_rho1)));
  }
  phi ~ normal(0,5);
  phi2 ~ normal(0,1);
  phi3 ~ normal(0.8, 0.05);
  ihr_noise ~ normal(1,0.8);
